...
const transformStudentData = (data: any[]): StudentSubmissionData[] => {
...
const transformActivityData = (data: any[]): ActivitySubmissionData[] => {
  return data.map((item) => ({
    id: item.id,
...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user }) => {
  ...
  const [topStudents, setTopStudents] = useState<StudentSubmissionData[]>([]);
  ...
  useEffect(() => {
    const fetchTeacherData = async () => {
      ...
      try {
        ...
        const { data: recentQuizzesData, error: recentQuizzesError } =
          await supabase
            .from("quizzes")
        ...
        if (quizIds && quizIds.length > 0) {
          ...
          const { data: topStudentsData, error: topStudentsError } =
            await supabase
              .from("quiz_submissions")
              .select(
                `
          ...
          setTopStudents(transformStudentData(topStudentsData || []));
          ...
          setRecentActivities(
            transformActivityData(recentActivitiesData || [])
          );
        } else {
          setTotalStudents(0);
        }
      ...
      <div className="bg-white p-6 rounded-lg shadow-md overflow-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {[
            ...
            <div
              key={index}
              className={`${stat.color} rounded-lg p-6 flex items-center`}
            >
                ...
                <li key={index} className="flex items-center">
                  ...
                  <div>
                    <p className="font-medium">
                      Quiz submitted - {activity.quizzes.title}
                    </p>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <ul className="space-y-4">
              ...
              {recentQuizzes.filter(
                (quiz) => new Date(quiz.release_date) > new Date()
              ).length === 0 && (
